...
class IpanemaViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    
    @IBOutlet weak var ipanemaTable: UITableView!
    ...
    override func viewDidLoad() {
        ...
        if let bundlePath = Bundle.main.path(forResource: "Ipanema", ofType: "plist"){
            if let dictionary = NSDictionary(contentsOfFile: bundlePath){
                
                bairros = dictionary["ipanema"] as? NSArray
                print(bairros?.count)
            }
        }
    }
    override func prepare(for next: UIStoryboardSegue, sender: Any?) {
        if next.identifier == "ligacao" {
            if let destination = next.destination as? LocalizacaoViewController {
                if let pontosIndex = (ipanemaTable.indexPathForSelectedRow as NSIndexPath?)?.row {
