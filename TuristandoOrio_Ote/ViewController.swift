...
class ViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    @IBOutlet weak var bairrograndeRio: UITableView!
    ...
    override func viewDidLoad() {
        ...
        if let bundlePath = Bundle.main.path(forResource: "BairrosGRio", ofType: "plist"){
            if let dictionary = NSDictionary(contentsOfFile: bundlePath){
                
                bairros = dictionary["bairrosgrio"] as? NSArray
                print(bairros?.count)
            }
        }
    }
    override func prepare(for next: UIStoryboardSegue, sender: Any?) {
        if next.identifier == "ligacao" {
            if let destination = next.destination as? LocalizacaoViewController {
                if let pontosIndex = (bairrograndeRio.indexPathForSelectedRow as NSIndexPath?)?.row {
