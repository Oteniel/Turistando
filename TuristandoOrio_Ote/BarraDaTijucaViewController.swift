//
//  BarraDaTijucaViewController.swift
//  TuristandoOrio_Ote
//
...
class BarraDaTijucaViewController:  UIViewController, UITableViewDataSource, UITableViewDelegate {
    
    @IBOutlet weak var barraTable: UITableView!
    ...
    override func viewDidLoad() {
        ...
        if let bundlePath = Bundle.main.path(forResource: "BarradaTijuca", ofType: "plist"){
            if let dictionary = NSDictionary(contentsOfFile: bundlePath){
                
                bairros = dictionary["barradatijuca"] as? NSArray
                print(bairros?.count)
            }
        }
    }
    override func prepare(for next: UIStoryboardSegue, sender: Any?) {
        if next.identifier == "ligacao" {
            if let destination = next.destination as? LocalizacaoViewController {
                if let pontosIndex = (barraTable.indexPathForSelectedRow as NSIndexPath?)?.row {
