//
//  BairrosRioViewController.swift
//  TuristandoOrio_Ote
//
...
class CentroRioViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {

    @IBOutlet weak var rioTable: UITableView!
    ...
    override func viewDidLoad() {
        ...
        if let bundlePath = Bundle.main.path(forResource: "BairrosRio", ofType: "plist"){
            if let dictionary = NSDictionary(contentsOfFile: bundlePath){
                
                bairros = dictionary["bairrosrio"] as? NSArray
                print(bairros?.count)
            }
        }
    }
    override func prepare(for next: UIStoryboardSegue, sender: Any?) {
        if next.identifier == "ligacao" {
            if let destination = next.destination as? LocalizacaoViewController {
                if let pontosIndex = (rioTable.indexPathForSelectedRow as NSIndexPath?)?.row {
